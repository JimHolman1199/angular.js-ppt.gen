export const removeNoise = (e, type) => {
  let { offsetParent:{offsetParent}, lastElementChild } = e.currentTarget;
  let inputNoise;
...
export const generateNoise = (e, type) => {
  e = e.currentTarget;
  ...
  for (let i = 0; i <= verticalNoiseToGenerateBottom; i++) {
    svg.appendChild(
      createSvg(Object.assign({}, commonVerticalConfig, {
  ...
  for (let i = 0; i <= verticalNoiseToGenerateTop; i++) {
    svg.appendChild(
      createSvg(
        Object.assign({}, commonVerticalConfig, {
        noiseWidth: Math.floor(Math.random() * (16 - 4) + 4),
        svgGroupX: Math.floor(Math.random() * (inputWidth - 1) + 1),
  ...
  for (let i = 0; i <= maxNumberOfVerticalNoise; i++) {
    svg.appendChild(
      createSvg(
        Object.assign({}, commonHorizontalConfig, {
